...
`):"_None yet. Save skills from the chat to reuse them._",this._view?.webview.postMessage({type:"assistantMessage",html:I(E)});return}default:break}}let a=S.workspace.getConfiguration("clawpilot"),l=a.get("model","llama3"),c=a.get("systemPrompt",""),d=i?null:e.match(/^\/(\w+)\s*(.*)/s),p=["plan","edit","fix","run","test","refactor","build","review","optimize","types"],u=o??!1;d&&(n=this._expandSlashCommand(d[1],d[2],t),p.includes(d[1])&&(u=!0));try{n=await this._resolveMentionsInMessage(n)}catch{}let g=n,v=[];if(this._contextRegistry)try{let h=await this._contextRegistry.assemble(n,8e3);h.text&&h.text.trim()&&(g=h.text+`
